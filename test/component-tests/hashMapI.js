...
contract('HashMapI', async (accounts) => {
    ...
    async function verifySingleHash(hashMapStateArray, idx) {
        ...
        const hashAtIdx = web3.utils.sha3(accounts[idx]);
    ...
    async function verifyHashMappingVariables(hashMapStateArray, firstIdx, nextIdx, count) {
        ...
        expect(firstIdx).to.be.equal(info.firstIdx.toNumber());
        expect(nextIdx).to.be.equal(info.nextIdx.toNumber());
        expect(count).to.be.equal(info.count.toNumber());
